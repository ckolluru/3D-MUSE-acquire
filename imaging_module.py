...
class imagingClass(QtCore.QThread):
	...
	def run(self):
		...
		while self.current_cut_index < self.num_cuts and self.threadActive:
			...
			self.statusBarSignal.emit('Additional cuts - End of cutting section ' + str(self.current_cut_index))
						
		# Log appropriately
		if self.threadActive:
			logging.info('Completed acquisition cycle, finished %s cuts', self.num_cuts)
			self.statusBarSignal.emit('Completed acquisition cycle, finished ' + str(self.num_cuts) + ' cuts')

		else:
			logging.info('Acquisition cycle stopped after %s cuts', self.current_cut_index)
			self.statusBarSignal.emit('Acquisition cycle stopped after ' + str(self.current_cut_index) + ' cuts')
